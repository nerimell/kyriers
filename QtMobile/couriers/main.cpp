...
int main(int argc, char *argv[]) {
    ...
    if(file.open(QIODevice::ReadOnly))eventhandler.isPin = true;
    ...
    if (memberString.size()!= 0)
    {
        ...
        foreach (QString encodedPair, keyValPairs) {
            ...
            if (keyVal[0] == "phone"){
                loginField.setText(keyVal.at(1).toStdString());
            }
            else if (keyVal[0] == "password")
                passField.setText(keyVal.at(1).toStdString());
            else if (keyVal[0] == "pin")
                MyEngine.user.setPin(keyVal.at(1).toStdString());
        }
    }
    file.close();
    */
